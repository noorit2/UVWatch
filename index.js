...
app.get('/countries', async (req, res) => {
  try {
    const response = await axios.get('http://api.geonames.org/countryInfoJSON?username=noorit&maxRows=1');
...
app.get("/select-city",async (req,res)=>{
  ...
  try {
    const response = await axios.get(`http://api.geonames.org/childrenJSON?geonameId=${geoID}&username=noorit`);
...
app.get("/View-UF",async (req,res)=>{
  ...
  const response = await axios.get(`https://api.openuv.io/api/v1/uv?lat=${lat}&lng=${log}`,{
    headers:{
      'x-access-token': "openuv-1ckmy5rlypudh7s-io",
    }
  });
